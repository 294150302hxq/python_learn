...
class Network(object):
    ...
    def SGD(self, train_data, min_batch_size, epoches, eta, test_data=False):
        ...
        min_batches = [train_data[k:k+min_batch_size] for k in xrange(0, n, min_batch_size)]  # 每一个batch里面有自己的数据
        for k in xrange(0, epoches):   # 利用同样的train进行多次循环
            random.shuffle(train_data)  # 打乱
    ...
    def updata_parameter(self, min_batch, eta):  # eta是学习率
        """1) 反向传播计算每个样本梯度值
           2） 累加每个批次样本的梯度值
           3） 权值更新"""
    ...
    def evaluate(self, test_data):
        ...
        z = [(np.argmax(self.forward(x)), y) for x, y in test_data]  # 得到识别结果和真实结果
